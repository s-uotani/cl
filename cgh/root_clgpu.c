...
int main(){
	...
	for(i=0; i<points; i++){
		...
		pls_z[i] = (float)z_buf*40+100000.0F;
	}
	fclose(fp);
	...
	kernel = clCreateKernel(
		program,			//プログラムオブジェクト
		"root_OpenCL",	//カーネルオブジェクト化するカーネル関数名
		NULL				//エラーコードを格納する変数
	);
	...
	fp = fopen("root_clgpu.bmp","wb");		//宣言したfpと使用するファイル名，その読み書きモードを設定．バイナリ(b)で書き込み(w)
	...
	printf("'root_clgpu.bmp' was saved.\n\n");
	fclose(fp);
