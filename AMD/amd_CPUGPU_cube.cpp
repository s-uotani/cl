...
int main(){
	...
	CPUkernel = clCreateKernel(CPUprogram, "kernel_source_cube", &status);
	error_CL(status);

	//GPU
	cl_kernel		GPUkernel = NULL;
	GPUkernel = clCreateKernel(GPUprogram, "kernel_source_cube", &status);
	error_CL(status);
	...
	printf("CPU Kernel Time(cube): %f [ms]\n", (CPUkernel_time / 1000000.0));
	printf("GPU Kernel Time(cube): %f [ms]\n\n", (GPUkernel_time / 1000000.0));
