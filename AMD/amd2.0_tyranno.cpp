...
int main(){
	...
	command_queue = clCreateCommandQueueWithProperties(context, device_id, props, &status);
	error_CL(status);
	...
	*/
	...
	kernel = clCreateKernel(program, "kernel_source_tyranno", &status);
	error_CL(status);
	...
	printf("Kernel Time(tyranno2.0): %f [ms]\n\n", (kernel_time / 1000000.0));
	...
	//ファイルに書き込み
	fp = fopen("amd2.0_tyranno.bmp", "wb");
