...
int main(){
	...
	if (fp == NULL){
		...
		for (i = 0; i < POINT_NUMBER; i++){
			...
			pls_z[i] = PI / (WAVELENGTH * 40 * ((float)z_data) + 50000);
	...
	//ソースコードの読み込み
	fp = fopen("kernel_source_ten.cl", "r");
	if (!fp){
		printf("カーネルの読み込みに失敗");
		exit(1);
	}
	...
	CPUkernel = clCreateKernel(CPUprogram, "kernel_source_ten", &status);
	error_CL(status);

	//GPU
	cl_kernel		GPUkernel = NULL;
	GPUkernel = clCreateKernel(GPUprogram, "kernel_source_ten", &status);
	error_CL(status);
	...
	printf("CPU Kernel Time(ten): %f [ms]\n", (CPUkernel_time / 1000000.0));
	printf("GPU Kernel Time(ten): %f [ms]\n\n", (GPUkernel_time / 1000000.0));
